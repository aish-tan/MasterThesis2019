...
library(lattice) #install.packages('directlabels')
...
#out_dir <- "/Users/aita2652/Cell Profiler Material/15MayLiveCell/R Output/NewOutput"
out_dir <- "/Users/aita2652/Cell Profiler Material/15MayLiveCell/NewOut1"
...
n <- 10
#foo <- arrange(y, NetRank)
#foo <- arrange(y, Rank.TP01)  #You can select top descriptors in whatever way you find can represent data well
#TopDes <- c(foo[1:n,1])
...
t1 <- ggplot(HDnuccyt, aes(x = TimePoint, y = HDnuccyt[,4], color = TreatConc)) + ylab(TopDes[i]) +
...
t2 <- ggplot(DBnuccyt, aes(x = TimePoint, y = DBnuccyt[,4], color = TreatConc)) + ylab(TopDes[i]) +
  geom_smooth( se = T, fullrange = T, aes(fill = TreatConc)) +
...
t3 <- ggplot(BPAnuccyt, aes(x = TimePoint, y = BPAnuccyt[,4], color = TreatConc)) + ylab(TopDes[i]) +
  geom_smooth( se = T, fullrange = T, aes(fill = TreatConc)) +
...
ggsave(filename = paste0(out_dir,"/NetTreatmentPlot_trytrytry",TopDes[i],".pdf"),
